...
import static us.byteb.advent.y19.Day3.*;
...
class Day3Test {
  ...
  void examplePart1(final String path1, final String path2, final long result) {
    final WirePath wp1 = WirePath.of(path1);
    final WirePath wp2 = WirePath.of(path2);
    System.out.println(print(wp1, wp2));

    final Point point =
        Point.shortestManhattanDistanceToCenter(WirePath.intersectionPoints(wp1, wp2));
    assertEquals(result, point.manhattanDistanceToCenter());
  }
}
